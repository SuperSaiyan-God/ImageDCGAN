...
class DCGAN(object):
    ...
    def discriminator_block(self, depth, input_shape=None, alpha=0.2, dropout=0.4):
        ...
        block = Sequential()
        if input_shape:
            block.add(Conv2D(depth, 5, strides=2,
...
class Image_DCGAN(object):
    def __init__(self, images, load_prev_model=False):
        ...
        self.DCGAN = DCGAN(rows=self.img_rows,
                           cols=self.img_cols, channels=self.channels)
        ...
        for i in range(train_steps):
            ...
            if save_interval > 0:
                if (i+1) % save_interval == 0:
                    ...
                    self.plot_images(
                        save2file=True, samples=noise_input.shape[0], noise=noise_input, step=(i+1))
