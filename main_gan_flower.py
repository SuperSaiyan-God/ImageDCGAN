...
def load_images(datapath, newsize):
    ...
    imarrays = []
    for image in images:
        # , grayscale=True) # uncomment for black and white images
        img = load_img(image)
        # Resize the images for speeding up computations
...
with tf.device('/GPU:0'):
    ...
    # Train the GAN
    if train_model:
        d_losses, a_losses, d_acc, a_acc = model_dcgan.train(
            train_steps=n_epochs, batch_size=batch_size, save_interval=500)

        # Save and plot the losses
        np.savetxt("outputs/losses.txt",
                   np.transpose([d_losses, a_losses, d_acc, a_acc]))
